<x-layout title="Manager - Products">
    @php
    ...
    <div class="container mx-auto p-6">
        <h1 class="text-2xl font-bold mb-6">Manager - Product Dashboard</h1>
                ...
                @forelse($products as $product)
                    ...
                    <tr>
                        <td class="border border-gray-300 px-4 py-2 text-center">
                            <img src="{{ asset('images/' . $product->image) }}" alt="{{ $product->name }}" class="w-20 h-20 object-contain mx-auto">
                        </td>
                        ...
                        @if(auth()->check() && in_array(auth()->user()->role, ['admin','manager']))
                            <td class="border border-gray-300 px-4 py-2 text-center space-x-2">
                                ...
                                <!-- Delete Button -->
                                <button class="bg-red-500 text-white px-3 py-1 my-2 rounded transition duration-500 hover:bg-[#FFD700]"
                                    data-id="{{ $product->id }}"
                    ...
                    <div id="sizes-wrapper" class="space-y-2">
                        @foreach($sizeNames as $key => $label)
                            <div class="flex items-center space-x-2">
                                ...
                                <input type="number" name="stock[{{ $key }}]" id="stock_{{ $key }}" placeholder="Enter stock change (use negative to decrease)" class="ml-4 px-2 py-1 border rounded w-48 hidden">
                            </div>
                        @endforeach
                    </div>
                </div>
