...
class AccountController extends Controller
{
    ...
    public function updateAccount(Request $request)
    {
        ...
        if (!empty($validatedData['password'])) {
            $user->password = $validatedData['password']; // Keeping original behavior
        }
    ...
    public function deleteAccount()
    {
        ...
        // Prevent deletion for admin and manager roles
        if (in_array($user->role, ['admin', 'manager'])) {
            return redirect()->route('account')->with('error', 'Admin and Manager accounts cannot be deleted.');
        }
